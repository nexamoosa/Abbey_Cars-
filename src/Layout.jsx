...
function Layout() {
  ...
  const isAreaPage = location.pathname.startsWith('/areas-we-cover')
  ...
  return (
    ...
    >
      ...
      <main className={`flex-1 ${isHome || isServiceDetail || isServicesPage || isAboutPage || isOurStoryPage || isAreaPage ? '' : 'py-8'}`} style={{ backgroundColor: 'var(--site-bg)' }}>
        <div className={`${isHome || isServiceDetail || isServicesPage || isAboutPage || isOurStoryPage || isAreaPage ? 'w-full px-0' : 'mx-auto w-full max-w-6xl px-4 sm:px-6 lg:px-8'}`}>
          <Outlet />
        </div>
      </main>
